...
function EditProfilePopup({ currentUser }) {
  ...
  const handleChangeEditProfilePopupStatus = event => {
    setIsEditProfilePopupOpen(event.detail)
  }
  ...
  React.useEffect(() => {
    addEventListener("changeEditProfilePopupStatus", handleChangeEditProfilePopupStatus);
    return () => removeEventListener("changeEditProfilePopupStatus", handleChangeEditProfilePopupStatus)
  }, []);
  ...
  return (
    <PopupWithForm
      isOpen={isEditProfilePopupOpen} onSubmit={handleSubmit} title="Редактировать профиль" name="edit"
    >
